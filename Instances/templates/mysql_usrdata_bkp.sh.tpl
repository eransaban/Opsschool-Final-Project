...
aws s3 cp s3://saban-sql-backup/replication /tmp/replica  --recursive

#import DB
mysql -uroot -p${sqlpassword} prac < /tmp/replica/prac.sql
masterip=$(cat /tmp/replica/ip.txt)
repcount=$(cat /tmp/replica/repcount.txt)
...
echo "relay-log               = /var/log/mysql/mysql-relay-bin.log" | tee -a /etc/mysql/mysql.conf.d/mysqld.cnf
#change DBname
sudo sed -i '/binlog_do_db/s/include_database_name/prac/' /etc/mysql/mysql.conf.d/mysqld.cnf
#change ID
sudo sed -i '/server-id/s/1/20/' /etc/mysql/mysql.conf.d/mysqld.cnf
sudo service mysql restart
#create permission for slave and start lock 
sudo mysql -e "CHANGE MASTER TO MASTER_HOST='$masterip',MASTER_USER='slave', MASTER_PASSWORD='${sqlpassword}', MASTER_LOG_FILE='mysql-bin.000001', MASTER_LOG_POS=  $repcount;
"
sudo mysql -e "START SLAVE;"
sudo mysql -e "SHOW SLAVE STATUS\G;" > /tmp/slavestatus.txt

#create Backup 
sudo mkdir /tmp/backup
wget https://raw.githubusercontent.com/eransaban/protest/master/backupjob.sh
mv backupjob.sh /tmp/backup/backupjob.sh

chmod +x /tmp/backup/backupjob.sh

echo "0 0 * * * /tmp/backup/backupjob.sh" | sudo tee -a /tmp/backup/mysqlbackup
sudo crontab /tmp/backup/mysqlbackup

sudo /tmp/backup/backupjob.sh
